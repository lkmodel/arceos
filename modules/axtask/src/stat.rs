...
numeric_enum_macro::numeric_enum! {
    ...
    pub enum TimerType {
        /// 表示目前没有任何计时器(不在`linux`规范中，是`os`自己规定的)
        NONE = -1,
        /// 统计系统实际运行时间
        REAL = 0,
        /// 统计用户态运行时间
...
impl TimeStat {
    ...
    pub fn new() -> Self {
        Self {
            ...
            kernel_tick: monotonic_time_nanos() as usize,
    ...
    pub fn clear(&mut self) {
        ...
        self.stime_ns = 0;
        self.user_tick = 0;
        self.kernel_tick = monotonic_time_nanos() as usize;
    }
    /// 从用户态进入内核态，记录当前时间戳，统计用户态时间
    pub fn switch_into_kernel_mode(&mut self, tid: isize) {
        let now_time_ns = monotonic_time_nanos() as usize;
