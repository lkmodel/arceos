...
use axlog::{info, warn};
use crate::{arch::{read_trapframe_from_kstack, write_trapframe_to_kstack}, config::{KERNEL_PROCESS_ID, SIGNAL_TRAMPOLINE}, signal::{
    action::{SigActionFlags, SignalDefault, SIG_DFL, SIG_IGN},
    info::SigInfo,
    signal_no::SignalNo,
    ucontext::{self, SignalStack, SignalUserContext},
...
pub fn handle_signals() {
    ...
    let mut sp = if action.sa_flags.contains(SigActionFlags::SA_ONSTACK)
        && signal_module.alternate_stack.flags != ucontext::SS_DISABLE
    {
