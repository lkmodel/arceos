...
| ... | Kernel 代码/数据位于此 |
| 0xFFFF_FFC0_8700_0000 (?)  | <- Kernel 区域结束 / App 区域起始 (地址可配置) |
| ... | App 代码/数据加载于此 |
| 0xFFFF_FFC0_8800_0000 (?)  | <- App 区域结束 (地址可配置) |
...
* 内存分布冲突：我们真正约定内核的范围，理论上而言， `xFFFF_FFC0_8700_0000` --- `xFFFF_FFC0_8800_0000` 的空间内核一样可使用，不过在现有的情况下，尚未遇到需要修正错误的情况。
* 线程控制块错误：我们通过 `pthread_self()` 返回线程控制块，但是存在一个致命问题，在于其返回的值似乎并不满足 musl 对线程控制块的假设，导致对返回的结构体进行访问的时候，会发生报错。根本性的解决方式是将 `ABI CALL` 替换为 `ABI-SYS CALL`
* `malloc` 分配问题：可确信的是，在单次分配过大内存的时候(2M)，可能会导致block的分配报错，猜测是因为过大的内存块耗尽了已经分配的堆或页。我们采用了在每次分配内存前，手动进行一次小内存分配方式，手动触发堆或页分配的 `layz` 初始化逻辑，暂时绕过问题。
...
`loader_lib` 的目标是发展成为 ArceOS 中一个核心且高度灵活的组件，负责 Linux 原生应用程序和共享库 (Lib) 的加载与管理，基于已有的 Linux 社区，为构建丰富多样的应用程序生态系统奠定基础。

* 广泛的应用兼容性: `loader_lib` 将能够无缝加载并执行使用 `musl` 和 `GCC` 工具链编译的各类应用程序。无论是基于静态链接还是动态链接的应用，都将在 ArceOS 环境中高效运行，从而显著扩展 ArceOS 的适用范围。
* 成熟的多应用支持: 在当前的基础上，我们将实现更为完善和高效的多应用管理机制。基于 `hypervisor` 实现更高级进程模型，以实现更强的隔离性和安全性。
* 灵活的库管理: `loader_lib` 将支持更精细化的共享库 (Lib) 管理。不同版本和配置的 `mocklibc` 或其他共享库将能够根据应用程序的需求进行加载和管理。应用包中可以包含多个 `Lib` 库，并支持按需加载，从而优化内存占用并提升系统的灵活性。最终目标是实现类似于组件化的库管理模式，使得 ArceOS 的功能能够以模块化的方式进行构建和扩展。
* 丰富的系统功能支持: 通过持续完善 `ABI-SYS CALL` 的覆盖范围，包括对更多的文件系统的支持，以及更多高级系统调用的实现，ArceOS 将能够运行更为复杂的应用程序。这将显著提升 ArceOS 的功能完整性和实用性。
* 强大的工具链支持: 除了现有的 `musl` 支持，未来还将全面支持 `GCC` 工具链，使得开发者能够利用更广泛的开发工具和库来构建 ArceOS 上的应用程序。
* 优化的性能与安全性: 在追求功能完善的同时，loader_lib 也将持续关注性能优化和安全性提升。通过更精细的内存管理、更安全的 `ABI CALL` 机制以及对潜在安全风险的持续评估和修复，确保 ArceOS 能够稳定可靠地运行各类应用程序。
