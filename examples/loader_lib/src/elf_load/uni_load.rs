...
pub fn modify_plt_for_lib(app_elf: &ElfBytes<LittleEndian>, lib_elf: &ElfBytes<LittleEndian>) {
...
pub fn modify_plt_for_app(app_elf: &ElfBytes<LittleEndian>, lib_elf: &ElfBytes<LittleEndian>) {
    let (app_dynsym_table, app_dynstr_table) = app_elf
        .dynamic_symbol_table()
        .expect("Failed to parse dynamic symbol table")
        .expect("ELF should have a dynamic symbol table");
    ...
    for app_rela_dyn in app_rela_dyns {
        ...
        match app_rela_type {
            ...
            R_RISCV_RELATIVE => {
                ...
                unsafe {
                    *((APP_START as u64 + app_rela_dyn.r_offset) as *mut usize) = new_value;
                }
                debug!(
