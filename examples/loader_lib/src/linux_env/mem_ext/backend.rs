...
//
// use crate::linux_env::axfs_ext::api::FileExt;
//
// /// File backend for Lazy load `MapArea`. `file` should be a file holding an offset value.
// /// Normally, `MemBackend` won't share a file with other things, so we use a `Box` here.
