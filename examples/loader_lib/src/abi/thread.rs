...
pub extern "C" fn abi_pthread_create(
    ...
    //    let adjusted_start_routine = start_routine;
    let adjusted_start_routine = unsafe {
        mem::transmute::<usize, extern "C" fn(arg: *mut c_void) -> *mut c_void>(
            (start_routine as usize) + APP_START,
        )
    };

    info!("{:p}", adjusted_start_routine);

    unsafe { sys_pthread_create(res, attr, adjusted_start_routine, arg) }
}
