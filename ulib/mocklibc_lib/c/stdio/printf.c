...
//         S('E') = DBL,   S('F') = DBL,   S('G') = DBL,  S('A') = DBL,     S('c') = INT,
//         S('C') = UINT,  S('s') = PTR,   S('S') = PTR,  S('p') = UIPTR,   S('n') = PTR,
...
// {
//     typedef int (*FnABI)(const char *s, size_t l);
//     long *abi_ptr = (long *)(abi_entry + 8 * SYS_OUT);
//     FnABI func = (FnABI)(*abi_ptr);
//     func(s, l);
// }
//
...
//     for (; l >= sizeof pad; l -= sizeof pad) out(pad, sizeof pad);
//     out(pad, l);
// }
//
...
// #endif
//
// static int fmt_fp(long double y, int w, int p, int fl, int t)
// {
...
//         if (y != y)
//             s = (t & 32) ? "nan" : "NAN";
//         pad(' ', w, 3 + pl, fl & ~ZERO_PAD);
//         out(prefix, pl);
//         out(s, 3);
//         pad(' ', w, 3 + pl, fl ^ LEFT_ADJ);
//         return MAX(w, 3 + pl);
//     }
//
//     y = frexpl(y, &e2) * 2;
...
//         pad(' ', w, pl + l, fl);
//         out(prefix, pl);
//         pad('0', w, pl + l, fl ^ ZERO_PAD);
//         out(buf, s - buf);
//         pad('0', l - (ebuf - estr) - (s - buf), 0, 0);
//         out(estr, ebuf - estr);
//         pad(' ', w, pl + l, fl ^ LEFT_ADJ);
//         return MAX(w, pl + l);
//     }
//     if (p < 0)
//         p = 6;
...
//             pad(' ', w, p, fl);
//             ws = arg.p;
//             for (i = 0; i < 0U + p && *ws && i + (l = wctomb(mb, *ws++)) <= p; i += l) out(mb,
//             l); pad(' ', w, p, fl ^ LEFT_ADJ); l = w > p ? w : p; continue;
//         case 'e':
//         case 'f':
//         case 'g':
//         case 'a':
